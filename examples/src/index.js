...

{
	...
	controlSingle({
		singleDown(e, x, y) {
			...
			prevX = x
			prevY = y
			labelElem.innerHTML = `<b>down</b><br>id=${e.pointerId}<br>x=${roundStr1(x)} y=${roundStr1(y)}`
			return true
		},
		singleMove(e, x, y) {
			labelElem.innerHTML = `<b>move</b><br>id=${e.pointerId}<br>x=${roundStr1(x)} y=${roundStr1(y)}`
			elemX += x - prevX
			elemY += y - prevY
		...
		singleUp(e) {
			labelElem.innerHTML = `<b>up</b><br>id=${e.pointerId}`
		},
...

{
	...
	controlDouble({
		singleDown(e, x, y, isSwitching) {
			prevX = x
			prevY = y
			label.innerHTML = `<b>down x1</b><br>id=${e.pointerId}<br>x=${roundStr1(x)} y=${roundStr1(y)}`
			elem.setPointerCapture(e.pointerId)
			return true
		},
		singleMove(e, x, y) {
			label.innerHTML = `<b>move x1</b><br>id=${e.pointerId}<br>x=${roundStr1(x)} y=${roundStr1(y)}`
			elemX += x - prevX
			elemY += y - prevY
			;[elemX, elemY] = clamp(elem, elemX, elemY)
			translate(elem, elemX, elemY, elemScale)
		...
		singleUp(e, isSwitching) {
			label.innerHTML = `<b>up x1</b><br>id=${e.pointerId}`
		},
		doubleDown(e, e0, e1, x0, y0, x1, y1) {
			label.innerHTML =
				`<b>down x2</b><br>id0=${e0.pointerId} id1=${e1.pointerId}` +
		...
		doubleMove(e, e0, e1, x0, y0, x1, y1) {
			label.innerHTML =
				`<b>move x2</b><br>id0=${e0.pointerId} id1=${e1.pointerId}` +
				`<br>x0=${roundStr1(x0)} y0=${roundStr1(y0)}` +
				`<br>x1=${roundStr1(x1)} y1=${roundStr1(y1)}`
			const curX = (x0 + x1) / 2
			const curY = (y0 + y1) / 2
		...
		doubleUp(e, e0, e1) {
			label.innerHTML = `<b>up x2</b><br>id0=${e0.pointerId} id1=${e1.pointerId}`
		},
	}).on({ startElem: elem, offsetElem: wrap })
